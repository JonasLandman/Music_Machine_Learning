...
void setup() 
{
  ...
  arduino = new Arduino(this, Arduino.list()[1], 57600); //creates an Arduino object
...
void draw() 
{
  ...
  for (i = 0; i <= 5; i++)
  {
    ...
    OscMessage analogInputMessage = new OscMessage("/wek/inputs/"+i); //an OSC message in created in the form 'analog/i'
...
void oscEvent(OscMessage theOscMessage) 
{
  ...
  for(i = 8; i <= 10; i++)
  {
    if(theOscMessage.addrPattern().equals("/wek/outptuts/"+i) == true) //if the osc message = /digital/i/ (i represents the pin number)
    {
      if(oscValue == 0)
      {
  ...
  for(i = 3; i <= 11; i++)
  {
    if (i == 3 || i == 5 || i == 6 || i == 11 )
    {
      if(theOscMessage.addrPattern().equals("/wek/outptuts/"+i) == true) //if the osc message = /pwm/i/ (i represents the pin number)
      {
